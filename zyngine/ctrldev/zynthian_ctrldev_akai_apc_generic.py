...
class DeviceConfig:
    ...
    BTN_LEFT                : int
    BTN_RIGHT               : int
    BTN_KNOB_CTRL_VOLUME    : int
    BTN_KNOB_CTRL_PAN       : int
    BTN_KNOB_CTRL_SEND      : int
    BTN_KNOB_CTRL_DEVICE    : int
    BTN_SOFT_KEY_START      : int
    BTN_SOFT_KEY_END        : int
    BTN_SOFT_KEY_CLIP_STOP  : int
    BTN_SOFT_KEY_SOLO       : int
    BTN_SOFT_KEY_MUTE       : int
    BTN_SOFT_KEY_REC_ARM    : int
    BTN_SOFT_KEY_SELECT     : int
    BTN_KNOB_1              : int
    BTN_KNOB_2              : int
    BTN_KNOB_3              : int
    BTN_KNOB_4              : int
...
# --------------------------------------------------------------------------
class zynthian_ctrldev_akai_apc(zynthian_ctrldev_zynmixer, zynthian_ctrldev_zynpad, DeviceConfig = dev_configs):

    dev_ids = dev_configs.dev_ids
    ...
    def _on_midi_event(self, ev):
        ...
        if evtype == EV_NOTE_ON:
            ...
            if note == dev_configs.BTN_SHIFT:
                return self._on_shift_changed(True)

            if self._is_shifted:
                old_handler = self._current_handler
                # Change global mode here
                if note == dev_configs.BTN_KNOB_CTRL_DEVICE:
                    self._current_handler = self._device_handler
                elif note in [dev_configs.BTN_KNOB_CTRL_PAN, dev_configs.BTN_KNOB_CTRL_VOLUME]:
                    self._current_handler = self._mixer_handler
                    self._padmatrix_handler.refresh()
                elif note == dev_configs.BTN_KNOB_CTRL_SEND:
                ...
                if self._current_handler == self._mixer_handler:
                    if note == dev_configs.BTN_SOFT_KEY_CLIP_STOP:
                        self._padmatrix_handler.enable_seqman(True)
                    elif dev_configs.BTN_SOFT_KEY_SOLO <= note <= dev_configs.BTN_SOFT_KEY_END:
                        self._padmatrix_handler.enable_seqman(False)

            # Padmatrix related events
            if self._current_handler == self._mixer_handler:
                if dev_configs.BTN_PAD_START <= note <= dev_configs.BTN_PAD_END:
                ...
                elif note == dev_configs.BTN_RECORD and not self._is_shifted:
                    return self._padmatrix_handler.on_record_changed(True)
                elif note == dev_configs.BTN_PLAY:
                ...
                elif (dev_configs.BTN_SOFT_KEY_START <= note <= dev_configs.BTN_SOFT_KEY_END
                      and not self._is_shifted):
                    row = note - dev_configs.BTN_SOFT_KEY_START
                    return self._padmatrix_handler.on_toggle_play_row(row)
                elif dev_configs.BTN_TRACK_1 <= note <= dev_configs.BTN_TRACK_8:
                    track = note - dev_configs.BTN_TRACK_1
                    self._padmatrix_handler.on_track_changed(track, True)
                    self._current_handler.note_on(note, vel, self._is_shifted)
                    self._padmatrix_handler.refresh()
                    return True
                elif note == dev_configs.BTN_STOP_ALL_CLIPS:
                    self._padmatrix_handler.note_on(note, vel, self._is_shifted)
            ...
            # Padmatrix related events
            if self._current_handler == self._mixer_handler:
                if note == dev_configs.BTN_RECORD:
...
class FeedbackLEDs:
    ...
    def control_leds_off(self):
        buttons = [
            dev_configs.BTN_UP, dev_configs.BTN_DOWN, dev_configs.BTN_LEFT, dev_configs.BTN_RIGHT, dev_configs.BTN_KNOB_CTRL_VOLUME,
            dev_configs.BTN_KNOB_CTRL_PAN, dev_configs.BTN_KNOB_CTRL_SEND, dev_configs.BTN_KNOB_CTRL_DEVICE,
            dev_configs.BTN_SOFT_KEY_CLIP_STOP, dev_configs.BTN_SOFT_KEY_MUTE, dev_configs.BTN_SOFT_KEY_SOLO,
            dev_configs.BTN_SOFT_KEY_REC_ARM, dev_configs.BTN_SOFT_KEY_SELECT,
        ]
        for btn in buttons:
            self.led_off(btn)

    def pad_leds_off(self):
        buttons = [btn for btn in range(dev_configs.BTN_PAD_START, dev_configs.BTN_PAD_END + 1)]
        for btn in buttons:
            self.led_off(btn)
...
class DeviceHandler(ModeHandlerBase):
    def __init__(self, state_manager, leds: FeedbackLEDs):
        ...
        self._btn_actions = {
            dev_configs.BTN_OPT_ADMIN:      ("MENU", "SCREEN_ADMIN"),
            dev_configs.BTN_MIX_LEVEL:      ("SCREEN_AUDIO_MIXER", "SCREEN_ALSA_MIXER"),
            dev_configs.BTN_CTRL_PRESET:    ("SCREEN_CONTROL", "PRESET", "SCREEN_BANK"),
            dev_configs.BTN_ZS3_SHOT:       ("SCREEN_ZS3", "SCREEN_SNAPSHOT"),
            dev_configs.BTN_PAD_STEP:       ("SCREEN_ZYNPAD", "SCREEN_PATTERN_EDITOR"),
            dev_configs.BTN_METRONOME:      ("TEMPO",),
            dev_configs.BTN_RECORD:         ("TOGGLE_RECORD",),
            dev_configs.BTN_PLAY: (
                lambda is_bold: [
                    "AUDIO_FILE_LIST" if is_bold else "TOGGLE_PLAY"
                ]
            ),
            dev_configs.BTN_STOP_ALL_CLIPS: (
                lambda is_bold: [
                    "ALL_SOUNDS_OFF" if is_bold else "STOP"
                ]
            ),
            dev_configs.BTN_KNOB_1: (lambda is_bold: [f"V5_ZYNPOT_SWITCH:0,{'B' if is_bold else 'S'}"]),
            dev_configs.BTN_KNOB_2: (lambda is_bold: [f"V5_ZYNPOT_SWITCH:1,{'B' if is_bold else 'S'}"]),
            dev_configs.BTN_KNOB_3: (lambda is_bold: [f"V5_ZYNPOT_SWITCH:2,{'B' if is_bold else 'S'}"]),
            dev_configs.BTN_KNOB_4: (lambda is_bold: [f"V5_ZYNPOT_SWITCH:3,{'B' if is_bold else 'S'}"]),
        }
    ...
    def refresh(self):
        ...
        self._leds.led_blink(dev_configs.BTN_KNOB_CTRL_DEVICE)

        # Lit up fixed buttons
        for btn in [dev_configs.BTN_PAD_UP, dev_configs.BTN_PAD_DOWN, dev_configs.BTN_PAD_LEFT, dev_configs.BTN_PAD_RIGHT]:
            self._leds.led_on(btn, COLOR_YELLOW, LED_BRIGHT_100)
        self._leds.led_on(dev_configs.BTN_SEL_YES, COLOR_GREEN, LED_BRIGHT_100)
        self._leds.led_on(dev_configs.BTN_BACK_NO, COLOR_RED, LED_BRIGHT_100)
        ...
        for btn in [dev_configs.BTN_F1, dev_configs.BTN_F2, dev_configs.BTN_F3, dev_configs.BTN_F4]:
            self._leds.led_on(btn, fn_color, LED_BRIGHT_100)
        self._leds.led_on(dev_configs.BTN_ALT, alt_color, LED_BRIGHT_100)
        ...
        if self._is_playing:
            self._leds.led_on(dev_configs.BTN_PAD_PLAY, COLOR_LIME, LED_BLINKING_8)
        if self._is_recording:
            self._leds.led_on(dev_configs.BTN_PAD_RECORD, COLOR_RED, LED_BLINKING_8)
    ...
    def note_on(self, note, velocity, shifted_override=None):
        ...
        if self._is_shifted:
            ...
            if note in (dev_configs.BTN_UP, dev_configs.BTN_PAD_UP):
                self._state_manager.send_cuia("ARROW_UP")
            elif note in (dev_configs.BTN_DOWN, dev_configs.BTN_PAD_DOWN):
                self._state_manager.send_cuia("ARROW_DOWN")
            elif note in (dev_configs.BTN_LEFT, dev_configs.BTN_PAD_LEFT):
                self._state_manager.send_cuia("ARROW_LEFT")
            elif note in (dev_configs.BTN_RIGHT, dev_configs.BTN_PAD_RIGHT):
                self._state_manager.send_cuia("ARROW_RIGHT")
            elif note == dev_configs.BTN_SEL_YES:
                self._state_manager.send_cuia("V5_ZYNPOT_SWITCH", [3, 'S'])
            elif note == dev_configs.BTN_BACK_NO:
                self._state_manager.send_cuia("BACK")
            elif note == dev_configs.BTN_ALT:
                self._is_alt_active = not self._is_alt_active
                self._state_manager.send_cuia("TOGGLE_ALT_MODE")
                self.refresh()
            else:
                # Function buttons (F1-F4)
                fn_btns = {dev_configs.BTN_F1: 1, dev_configs.BTN_F2: 2, dev_configs.BTN_F3: 3, dev_configs.BTN_F4: 4}
    ...
    def cc_change(self, ccnum, ccval):
        ...
        zynpot = {
            dev_configs.KNOB_LAYER: 0,
            dev_configs.KNOB_BACK: 1,
            dev_configs.KNOB_SNAPSHOT: 2,
            dev_configs.KNOB_SELECT: 3
        }.get(ccnum, None)
        if zynpot is None:
            return
    ...
    def on_screen_change(self, screen):
        screen_map = {
            "option":         (dev_configs.BTN_OPT_ADMIN, 0),
            "main_menu":      (dev_configs.BTN_OPT_ADMIN, 0),
            "admin":          (dev_configs.BTN_OPT_ADMIN, 1),
            "audio_mixer":    (dev_configs.BTN_MIX_LEVEL, 0),
            "alsa_mixer":     (dev_configs.BTN_MIX_LEVEL, 1),
            "control":        (dev_configs.BTN_CTRL_PRESET, 0),
            "engine":         (dev_configs.BTN_CTRL_PRESET, 0),
            "preset":         (dev_configs.BTN_CTRL_PRESET, 1),
            "bank":           (dev_configs.BTN_CTRL_PRESET, 1),
            "zs3":            (dev_configs.BTN_ZS3_SHOT, 0),
            "snapshot":       (dev_configs.BTN_ZS3_SHOT, 1),
            "zynpad":         (dev_configs.BTN_PAD_STEP, 0),
            "pattern_editor": (dev_configs.BTN_PAD_STEP, 1),
            "arranger":       (dev_configs.BTN_PAD_STEP, 1),
            "tempo":          (dev_configs.BTN_METRONOME, 0),
        }
    ...
    def _handle_timed_button(self, btn, press_type):
        if press_type == CONST.PT_LONG:
            cuia = {
                dev_configs.BTN_OPT_ADMIN:   "POWER_OFF",
                dev_configs.BTN_CTRL_PRESET: "PRESET_FAV",
                dev_configs.BTN_PAD_STEP:    "SCREEN_ARRANGER",
            }.get(btn)
            if cuia:
...
class MixerHandler(ModeHandlerBase):
    ...
    main_chain_knob = dev_configs.KNOB_1
    ...
    def refresh(self):
        ...
        if self._is_shifted:
            # Knob Ctrl buttons
            btn = {
                FN_VOLUME: dev_configs.BTN_KNOB_CTRL_VOLUME,
                FN_PAN: dev_configs.BTN_KNOB_CTRL_PAN,
            }[self._knobs_function]
            self._leds.led_on(btn)

            # Soft Keys buttons
            btn = {
                FN_SEQUENCE_MANAGER: dev_configs.BTN_SOFT_KEY_CLIP_STOP,
                FN_MUTE: dev_configs.BTN_SOFT_KEY_MUTE,
                FN_SOLO: dev_configs.BTN_SOFT_KEY_SOLO,
                FN_SELECT: dev_configs.BTN_SOFT_KEY_SELECT,
                FN_SCENE: dev_configs.BTN_SOFT_KEY_REC_ARM,
            }[self._track_buttons_function]
            self._leds.led_on(btn)

            # Clips bank selection
            btn = dev_configs.BTN_LEFT if self._chains_bank == 0 else dev_configs.BTN_RIGHT
            self._leds.led_on(btn)
        ...
        # Otherwise, show current function status
            ...
            if self._track_buttons_function == FN_SCENE:
                for i in range(8):
                    ...
                    self._leds.led_state(dev_configs.BTN_TRACK_1 + i, state)
                return

            if self._track_buttons_function == FN_SEQUENCE_MANAGER:
                self._leds.led_blink(dev_configs.BTN_SOFT_KEY_CLIP_STOP)
                return
            ...
            for i in range(8):
                ...
                if chain.chain_id == 0:
                    continue
                self._leds.led_state(dev_configs.BTN_TRACK_1 + i, query(index))
    ...
    def note_on(self, note, velocity, shifted_override=None):
        ...
        if self._is_shifted:
            if note == dev_configs.BTN_KNOB_CTRL_VOLUME:
                self._knobs_function = FN_VOLUME
            elif note == dev_configs.BTN_KNOB_CTRL_PAN:
                self._knobs_function = FN_PAN
            elif note == dev_configs.BTN_SOFT_KEY_MUTE:
                self._track_buttons_function = FN_MUTE
            elif note == dev_configs.BTN_SOFT_KEY_SOLO:
                self._track_buttons_function = FN_SOLO
            elif note == dev_configs.BTN_SOFT_KEY_REC_ARM:
                self._track_buttons_function = FN_SCENE
            elif note == dev_configs.BTN_SOFT_KEY_CLIP_STOP:
                self._track_buttons_function = FN_SEQUENCE_MANAGER
            elif note == dev_configs.BTN_LEFT:
                self._chains_bank = 0
            elif note == dev_configs.BTN_RIGHT:
                self._chains_bank = 1
            elif note == dev_configs.BTN_STOP_ALL_CLIPS:
                self._stop_all_sounds()
            elif note == dev_configs.BTN_PLAY:
                self._run_track_button_function_on_channel(255, FN_MUTE)
            elif note == dev_configs.BTN_SOFT_KEY_SELECT:
                self._track_buttons_function = FN_SELECT
            elif note == dev_configs.BTN_RECORD:
                self._state_manager.send_cuia("TOGGLE_RECORD")
                return True  # skip refresh
            elif note == dev_configs.BTN_UP:
                self._state_manager.send_cuia("BACK")
                return True  # skip refresh
            elif note == dev_configs.BTN_DOWN:
        ...
        # Otherwise, handle primary functions
            ...
            if dev_configs.BTN_TRACK_1 <= note <= dev_configs.BTN_TRACK_8:
    ...
    def update_strip(self, chan, symbol, value):
        ...
        if 0 > chan > 8:
            return
        self._leds.led_state(dev_configs.BTN_TRACK_1 + chan, value)
        return True
    ...
    def _update_control(self, type, ccnum, ccval, minv, maxv):
        if self._is_shifted:
            ...
            index = (ccnum - dev_configs.KNOB_1) + self._chains_bank * 8
            chain = self._chain_manager.get_chain_by_index(index)
    ...
    def _run_track_button_function(self, note):
        index = (note - dev_configs.BTN_TRACK_1) + self._chains_bank * 8
...
class PadMatrixHandler(ModeHandlerBase):
    ...
    def __init__(self, state_manager, leds: FeedbackLEDs):
        ...
        for c in reversed(range(self._cols)):
            for r in range(self._rows):
                self._pads.append(dev_configs.BTN_PAD_END - (r * self._cols + c))
    ...
    def on_track_changed(self, track, state):
        ...
        if state and self._seqman_func is not None and not self._is_shifted:
            btn = dev_configs.BTN_TRACK_1 + track

            if btn == dev_configs.BTN_LEFT:
                return self._change_scene(-1)
            if btn == dev_configs.BTN_RIGHT:
                return self._change_scene(1)

            func = {
                dev_configs.BTN_KNOB_CTRL_VOLUME: FN_COPY_SEQUENCE,
                dev_configs.BTN_KNOB_CTRL_PAN: FN_MOVE_SEQUENCE,
                dev_configs.BTN_KNOB_CTRL_SEND: FN_CLEAR_SEQUENCE,
            }.get(btn)
            if func is not None:
    ...
    def note_on(self, note, velocity, shifted_override=None):
        self._on_shifted_override(shifted_override)
        if not self._is_shifted:
            if note == dev_configs.BTN_STOP_ALL_CLIPS:
                self._btn_timer.is_pressed(note, time.time())
    ...
    def _handle_timed_button(self, btn, ptype):
        if btn == dev_configs.BTN_STOP_ALL_CLIPS:
    ...
    def _refresh_tool_buttons(self):
        ...
        if self._seqman_func is not None:
            active = {
                FN_COPY_SEQUENCE: dev_configs.BTN_KNOB_CTRL_VOLUME,
                FN_MOVE_SEQUENCE: dev_configs.BTN_KNOB_CTRL_PAN,
                FN_CLEAR_SEQUENCE: dev_configs.BTN_KNOB_CTRL_SEND,
            }[self._seqman_func]
            for idx in range(8):
                btn = dev_configs.BTN_TRACK_1 + idx
        ...
        for row in range(5):
            ...
            self._leds.led_state(dev_configs.BTN_SOFT_KEY_START + row, state)
...
class StepSeqHandler(ModeHandlerBase):
    ...
    def __init__(self, state_manager, leds: FeedbackLEDs, dev_idx):
        ...
        for r in range(self.PAD_ROWS):
            for c in reversed(range(self.PAD_COLS)):
                self._pads.append(dev_configs.BTN_PAD_END - (r * self.PAD_COLS + c))
    ...
    def _refresh_status_leds(self):
        ...
        if self._is_shifted:
            ...
            self._leds.led_on(dev_configs.BTN_KNOB_CTRL_SEND)

            if self._note_config is None:
                self._leds.led_on(dev_configs.BTN_UP + self._note_page_number)

        if self._is_arranger_mode:
            self._leds.led_blink(dev_configs.BTN_SOFT_KEY_SELECT)
    ...
    def _refresh_note_pads(self):
        ...
        for idx, note_spec in self._note_pads.items():
            pad = dev_configs.BTN_PAD_START + idx
    ...
    def note_on(self, note, velocity, shifted_override=None):
        ...
        if self._is_shifted:
            ...
            if note == dev_configs.BTN_KNOB_CTRL_SEND:
                self.refresh()
            elif note == dev_configs.BTN_STOP_ALL_CLIPS:
                self._stop_all_sounds()
            ...
            elif dev_configs.BTN_PAD_START <= note <= dev_configs.BTN_PAD_START + 7:
            ...
            if dev_configs.BTN_SOFT_KEY_START <= note <= dev_configs.BTN_SOFT_KEY_END:
                control = None
                if self._note_config is not None:
                ...
                if note == dev_configs.BTN_SOFT_KEY_MUTE:
                    if self._is_arranger_mode:
            ...
            if note == dev_configs.BTN_PLAY:
                self._libseq.togglePlayState(self._zynseq.bank, self._selected_seq)

            elif dev_configs.BTN_PAD_START <= note <= dev_configs.BTN_PAD_END:
                self._pressed_pads[note] = time.time()
                if dev_configs.BTN_PAD_START + 8 <= note <= dev_configs.BTN_PAD_END:
                ...
                if self._note_config is not None:
                    return self._note_config.note_on(note, velocity, self._is_shifted)
                if dev_configs.BTN_PAD_START <= note <= dev_configs.BTN_PAD_START + 7:
            ...
            elif note == dev_configs.BTN_KNOB_CTRL_VOLUME:
            ...
            elif note == dev_configs.BTN_LEFT and self._note_config is None:
                if self._is_arranger_mode:
                    self._change_to_previous_pattern()
                else:
                    self._change_instruments_page(-1)
            elif note == dev_configs.BTN_RIGHT and self._note_config is None:
                if self._is_arranger_mode:
                    self._change_to_next_pattern()
                else:
                    self._change_instruments_page(1)
    ...
    def note_off(self, note, shifted_override=None):
        ...
        if dev_configs.BTN_PAD_START <= note <= dev_configs.BTN_PAD_END:
            ...
            if not self._pressed_pads:
                self._pressed_pads_action = None
        elif note in (dev_configs.BTN_STOP_ALL_CLIPS, dev_configs.BTN_SOFT_KEY_MUTE):
            if self._is_arranger_mode:
    ...
    def cc_change(self, ccnum, ccval):
        ...
        if self._pressed_pads:
            ...
            adjust_pad_func = {
                dev_configs.KNOB_1: self._update_note_pad_duration,
                dev_configs.KNOB_2: self._update_note_pad_velocity,
                dev_configs.KNOB_3: self._update_note_pad_stutter_count,
                dev_configs.KNOB_4: self._update_note_pad_stutter_duration,
            }.get(ccnum)
            adjust_step_func = {
                dev_configs.KNOB_1: self._update_step_duration,
                dev_configs.KNOB_2: self._update_step_velocity,
                dev_configs.KNOB_3: self._update_step_stutter_count,
                dev_configs.KNOB_4: self._update_step_stutter_duration,
            }.get(ccnum)
    ...
    def _remove_note_pad(self, pad):
        idx = pad - dev_configs.BTN_PAD_START
    ...
    def _run_note_pad_action(self, note, velocity=None, state=True):
        dst_idx = note - dev_configs.BTN_PAD_START
    ...
    def _change_instruments_page(self, offset):
        ...
        indicator = dev_configs.BTN_TRACK_1 + self._note_page_number
    ...
    def _change_instrument(self, pad):
        index = pad - dev_configs.BTN_PAD_START
    ...
    def _play_note_pad(self, pad, velocity=None, on=True, force=False):
        ...
        note_spec = self._note_pads.get(pad - dev_configs.BTN_PAD_START)
        if note_spec is None:
            return
    ...
    def _show_patterns_bar(self, overlay=True):
        for i in range(8):
            pad = dev_configs.BTN_PAD_START + i
            color = COLOR_WHITE
            mode = LED_BRIGHT_10
    ...
    def _create_note_control(self, kind, note):
        ...
        notes = {}
        row_led = None
        if note == dev_configs.BTN_SOFT_KEY_SELECT:
            row_led = note
            notes = self._note_pads

        elif dev_configs.BTN_SOFT_KEY_CLIP_STOP <= note <= dev_configs.BTN_SOFT_KEY_REC_ARM:
            row = note - dev_configs.BTN_SOFT_KEY_CLIP_STOP
...
class BaseControl:
    ...
    def note_on(self, note, velocity, shifted_override=None):
        col = (note - dev_configs.BTN_PAD_START) % self.PAD_COLS
        note_spec = self._notes.get(col)
        if note_spec is None:
            return

        row = (note - dev_configs.BTN_PAD_START) // self.PAD_COLS
...
class VelocityControl(BaseControl):
    KIND            = "velocity"
    INDICATOR_LED   = dev_configs.BTN_KNOB_CTRL_VOLUME
    COLOR           = COLOR_AQUA
...
class StutterCountControl(BaseControl):
    KIND            = "stutter-count"
    INDICATOR_LED   = dev_configs.BTN_KNOB_CTRL_SEND
